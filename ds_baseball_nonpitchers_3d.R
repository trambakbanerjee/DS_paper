...
baseball<-bball
dat.1<-baseball[baseball$year<2012 &baseball$pitcher==0,]
...
dat<- cbind(scale(sqrt(multi)*xbar),scale(S2.sample),scale(m))
dat_u<- cbind(scale(sqrt(multi)*ubar),scale(S2.u),scale(m))
Omegainv<- cov(dat)
...
for(jj in 1:length(eta.grid)){
  ...
  save.image("baseball_nonpitchers_3d.RData")
