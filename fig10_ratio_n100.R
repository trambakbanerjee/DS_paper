...
exp_2<-function(n,M,sigma2_u,reps){
  ...
  alpha<-0.5
  eta.grid<-c(0.25,0.5,1,1.5,2,2.5,3)
  
  risk.ds_orc<-matrix(0,reps,length(sigma2_u))
  risk.ds_mcv<-matrix(0,reps,length(sigma2_u))
  risk.ds_orc_var<-matrix(0,reps,length(sigma2_u))
  risk.ds_mcv_var<-matrix(0,reps,length(sigma2_u))
  ...
  risk.km.var<-matrix(0,reps,length(sigma2_u))
  ...
  for(j in 1:length(sigma2_u)){
    ...
    result<-foreach(r = 1:reps,.packages=c("isotone","RcppArmadillo","Rcpp","RcppProgress","REBayes","RSpectra","Rfast"))%dopar%{
      ...
      set.seed(r)
      sigma2<- runif(N,0.1,uu)
      set.seed(r^2)
      ee<-sample(c(-1,1),N,replace = T)
      set.seed(100*r)
      theta<- ee*0.5*(sigma2)+rnorm(N,0,0.5)
      X<-lapply(1:N,function(i) norm.rand(m[i],theta[i],sqrt(sigma2[i]),r*i))
      
      xbar<- sapply(1:N,function(i) mean(X[[i]]))
      S2.sample<- sapply(1:N,function(i)var(X[[i]]))
      S.sample<-sqrt(S2.sample)
      
      ratio.thetatau<-theta*sqrt(m/sigma2)
      ...
      risk.gl_r<- sum((ratio.thetatau-(sqrt(m)*thetahat.gl/S.sample))^2)/N
      
      #2. XKB - 3 versions
      thetahat.xkb.g<- thetahat.G(xbar,S2.sample/m)
      risk.xkb_g<- sum((ratio.thetatau-(sqrt(m)*thetahat.xkb.g/S.sample))^2)/N
      
      thetahat.xkb.sg<- thetahat.SG(xbar,S2.sample/m)
      risk.xkb_sg<- sum((ratio.thetatau-(sqrt(m)*thetahat.xkb.sg/S.sample))^2)/N
      
      thetahat.xkb.m<- thetahat.M(xbar,S2.sample/m)
      risk.xkb_m<- sum((ratio.thetatau-(sqrt(m)*thetahat.xkb.m/S.sample))^2)/N
      
      #3. KM
      X.vec<-unlist(X)
      id.km<-rep(1:N,m)
      km<- tryCatch(WGLVmix(X.vec,id.km,rep(1,length(X.vec))),
                    error=function(e) NULL)
      thetahat.km<- sqrt(m)*(km$dx/sqrt(km$dy))
      if(is.null(thetahat.km)){
        thetahat.km<-sqrt(m)*xbar/S.sample
      }
      risk.km_r<- sum((ratio.thetatau-thetahat.km)^2)/N
      risk.km_var<- sum((sigma2-km$dy)^2)/N
      ...
      for(jj in 1:length(eta.grid)){
        ...
        out_2<-ksd_md_cpp(dat,K_md,Omega[1,1],Omega[1,2],Omega[2,2],eta.grid[jj])
        ...
        loss_true_md[jj]<-sum((ratio.thetatau-(sqrt(m*tauhat)*deltaDS))^2)/N
      ...
      risk.ds_md_orc_r<- sum((ratio.thetatau-(sqrt(m*tauhat)*deltaDS))^2)/N
      risk.ds_md_orc_var<- sum((sigma2-1/tauhat)^2)/N
      
      ii<- min(which(loss_mcv_md==min(loss_mcv_md,na.rm = T)))
      eta_mcv_md<- eta.grid[ii]
      tauhat<- (m-3)/((m-1)*S2.sample)-(2/(m-1))*rho2_2[,ii]
      deltaDS<- xbar+(1/(m*tauhat))*rho1_2[,ii]
      risk.ds_md_mcv_r<- sum((ratio.thetatau-(sqrt(m*tauhat)*deltaDS))^2)/N
      risk.ds_md_mcv_var<- sum((sigma2-1/tauhat)^2)/N
      
     return(list("risk.gl"=risk.gl_r,
                  "risk.xkb.g"=risk.xkb_g,"risk.xkb.sg"=risk.xkb_sg,"risk.xkb.m"=risk.xkb_m,
                  "risk.km"=risk.km_r,
                 "risk.km.var"=risk.km_var,
                  "risk.ds_md_orc"=risk.ds_md_orc_r,
                  "risk.ds_md_mcv"=risk.ds_md_mcv_r,
                 "risk.ds_md_orc_var"=risk.ds_md_orc_var,
                 "risk.ds_md_mcv_var"=risk.ds_md_mcv_var,
    ...
    risk.ds_orc_var[,j]<-sapply(1:reps,function(i) result[[i]]$risk.ds_md_orc_var)
    risk.ds_mcv_var[,j]<-sapply(1:reps,function(i) result[[i]]$risk.ds_md_mcv_var)
    ...
    risk.km.var[,j]<-sapply(1:reps,function(i) result[[i]]$risk.km.var)
  ...
  result<-list("risk.ds_orc"=risk.ds_orc,"risk.ds_mcv"=risk.ds_mcv,
               "risk.ds_orc_var"=risk.ds_orc_var,"risk.ds_mcv_var"=risk.ds_mcv_var,
               "risk.gl"=risk.gl,"risk.km"=risk.km,"risk.km.var"=risk.km.var,
               "risk.xkbg"=risk.xkb.g,
...
g1<-ggplot()+geom_line(data=plotdata1,aes(x=u,y=Risk,color=type))+
  geom_point(data=plotdata1,aes(x=u,y=Risk,color=type,shape=type),fill=NA)+
  theme_bw()+
...
g2<-ggplot()+geom_line(data=plotdata2,aes(x=u,y=Risk,color=type))+
  geom_point(data=plotdata2,aes(x=u,y=Risk,color=type,shape=type),fill=NA)+
  theme_bw()+
  theme(legend.position='top',legend.title=element_blank(),
        legend.background = element_rect(fill="white",
                                         size=1, linetype="solid", colour ="black"),
...
g3<-ggplot()+geom_line(data=plotdata3,aes(x=u,y=Risk,color=type))+
  geom_point(data=plotdata3,aes(x=u,y=Risk,color=type,shape=type),fill=NA)+
  theme_bw()+
  theme(legend.position='top',legend.title=element_blank(),
        legend.background = element_rect(fill="white",
                                         size=1, linetype="solid", colour ="black"),
...
save.image(paste(getwd(),'/fig10_n100.RData',sep=''))
