...
baseball<-bball
dat.1<-baseball[baseball$year<2012 & baseball$pitcher==1,]
...
TSE.km_ind<- sum((theta-thetahat.km_ind[idy])^2)
...
TSE.km_dep<- sum((theta-thetahat.km_dep[idy])^2)
...
for(i in 1:length(idy)){
  ...
  loss1.naive[i]<- (theta[i]-asin(sqrt((temp$H+0.25)/(temp$AB+0.5))))^2
...
TSE.histunwt<- sum((theta-xbar.unwt[idy])^2)
...
#7. Double Shrinkage  
eta.grid<-seq(0.05,0.1,length.out = 50)
...
for(jj in 1:length(eta.grid)){
  ...
  TSE.ds_md_orc<- sum((theta-deltaDS_orc[idy])^2)
  ...
  TSE.ds_md_mcv<- sum((theta-deltaDS[idy])^2)
  ...
  save.image("baseball_pitchers_3d.RData")
