...
exp_1<-function(n,M,sigma2_u,reps){
  
  
  alpha<-0.5
  eta.grid<-c(0.5,1,1.5,2,2.5,3,3.5)
  ...
  for(j in 1:length(sigma2_u)){
    ...
    if(uu>1){
      ...
      eta.grid<-c(1,1.5,2,2.5,3,3.5,4)
    }
    ...
    result<-foreach(r = 1:reps,.packages=c("isotone","RcppArmadillo","Rcpp","RcppProgress","REBayes","RSpectra","Rfast"))%dopar%{
      ...
      p<-runif(N)
      set.seed(r)
      A<- runif(N,0.5,uu)#runif(N,0.1,uu)
      set.seed(r^2)
      ee<-sample(c(-1,1),N,replace = T)
      set.seed(100*r)
      theta<- (p<=0.7)*c(rmvnorm(1,rep(0,N),0.1*diag(N)))+(p>0.7)*c(rmvnorm(1,ee*rep(1,N),3*diag(N)))#(p<=0.5)*rep(0,N)+(p>0.5)*c(rmvnorm(1,ee*A/2,diag(N)))
      X<-lapply(1:N,function(i) norm.rand(m[i],theta[i],sqrt(A[i]),r*i))
      ...
      X.vec<-unlist(X)
      id.km<-rep(1:N,m)
      thetahat.km<- tryCatch(WTLVmix(X.vec,id.km,rep(1,length(X.vec)))$dx,
...
out_m10<-exp_1(n,M[1],sigma2_u,reps)
out_m30<-exp_1(n,M[2],sigma2_u,reps)
out_m50<-exp_1(n,M[3],sigma2_u,reps)
...
save.image(paste(getwd(),'/fig1.RData',sep=''))
