...
exp_4<-function(n,M,sigma2_u,reps){
  
  
  alpha<-0.5
  eta.grid<-c(1.1,1.3,1.5,3,5)
  ...
  for(j in 1:length(sigma2_u)){
    ...
    result<-foreach(r = 1:reps,.packages=c("rmutil","isotone","RcppArmadillo","Rcpp","RcppProgress","REBayes","RSpectra","Rfast"))%dopar%{
      ...
      A<- runif(N,0.1,uu)
      set.seed(100*r)
      theta<- (p<=0.8)*c(rmvnorm(1,A/2,0.5*diag(N)))+(p>0.8)*c(rmvnorm(1,2*A,2*diag(N)))
      X<-lapply(1:N,function(i) normlap.rand(m[i],1,theta[i],sqrt(A[i]),r*i))
      ...
      maxeigenval<-eigs_sym(K_md, 1, opts = list(retvec = FALSE))$values
      eta_md<-(maxeigenval-50*0.001)/(50-1)
...
out_m10<-exp_4(n,M[1],sigma2_u,reps)
out_m30<-exp_4(n,M[2],sigma2_u,reps)
out_m50<-exp_4(n,M[3],sigma2_u,reps)
...
save.image(paste(getwd(),'/fig5.RData',sep=''))
